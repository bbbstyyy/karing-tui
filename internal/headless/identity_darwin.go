...
// processIdentity 用 kern.proc.pid 读取进程启动时间作为身份 token。
//
...
func processIdentity(pid int) (ProcessIdentity, error) {
	...
	if err != nil {
		...
		return ProcessIdentity{}, fmt.Errorf("读取进程 %d 身份失败: %w", pid, err)
	}
	tv := kp.Proc.P_starttime
	return ProcessIdentity{PID: pid, Token: fmt.Sprintf("%d:%d", tv.Sec, tv.Usec)}, nil
}

// processGroupID 从同一份 kinfo_proc 里取进程组 ID（Eproc.Pgid）。
