...
func (m *Manager) testNodes(ctx context.Context, nodes []*config.Node, testURL string, timeoutMS int) (map[int64]int64, error) {
	...
	for _, n := range nodes {
		...
		tasks = append(tasks, delayTask{id: n.ID, tag: tag})
	}
	if len(outbounds) == 0 {
	...
	cfgPath := filepath.Join(m.Paths.Runtime, "latency.json")
	...
	for _, t := range tasks {
		...
		go func(t delayTask) {
			...
			sem <- struct{}{}
			defer func() { <-sem }()
			ms := queryDelay(client, apiPort, t.tag, testURL, timeoutMS)
			mu.Lock()
			results[t.id] = ms
			mu.Unlock()
		}(t)
...
func queryDelay(client *http.Client, apiPort int, tag, testURL string, timeoutMS int) int64 {
